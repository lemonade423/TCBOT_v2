...
def build_dynamic_explanations(groups: dict[str, pd.DataFrame]) -> str:
    ...
    for feature_name, df in groups.items():
        ...
        # 그룹 전체 텍스트 수집 (처음 보는 함수도 첫 행 복사 금지)
        merged_text = " ".join(
...
{full_code}
    ...
    # [FIX] 결과 표시: 헤더 문구 + 동적 설명 섹션(요청 반영)
    ...
    # [ADD] 폴백: 예외 시에도 버튼이 반드시 살아있도록 샘플 바이트 사용
...
"""
    ...
    log_file = st.file_uploader("📂 에러 로그 파일 업로드 (.log, .txt)", type=["log", "txt]", "txt"], key="log_file")
